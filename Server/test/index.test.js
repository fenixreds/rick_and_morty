...
const session = require('supertest');//para generar server ficticio
const agent = session(app);//para levantar server fake
...
describe('Test de RUTAS', ()=>{
    ...
    describe('DELETE /rickandmorty/fav/:id', ()=>{
        ...
        it('Returna un arreglo con los 2 chars previos', async ()=>{
            const { body } = await agent.delete('/rickandmorty/fav/256');
            expect(body.length).toEqual(2);
        });
        it('Debe retornar los personajes previos', async ()=>{
            await agent.post('/rickandmorty/fav').send(fakeBody3);
            const { body } = await agent.delete('/rickandmorty/fav/35');
            expect(body.length).toEqual(2);
        });
    });
});
